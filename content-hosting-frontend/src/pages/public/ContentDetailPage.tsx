...
export default function ContentDetailPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // Fetch the row by id. RLS will make unpublished/deleted rows invisible.
      const { data, error } = await supabase
        .from('content')
        .select('id,title,description,content_type,status,deleted_at,rich_text_content,audio_file_url,published_at')
        .eq('id', id)
        .single();
      ...
      // Extra safety: only render if it's publishable to the public
      if (data.status !== 'published' || data.deleted_at) {
        setLoading(false);
        return;
      }
      ...
      // If podcast, resolve an audio URL
      if (data.content_type === 'podcast' && data.audio_file_url) {
        // PRIVATE bucket: signed URL (recommended)
        const { data: signed, error: sErr } = await supabase
          .storage
          .from('podcasts')
          .createSignedUrl(data.audio_file_url, 60 * 60); // 1 hour
        if (!sErr && signed?.signedUrl) setAudioSrc(signed.signedUrl);

        // PUBLIC bucket alternative:
        // const { data: pub } = supabase.storage.from('podcasts').getPublicUrl(data.audio_file_url);
        // setAudioSrc(pub.publicUrl);
      }
  ...
  if (!item) {
    return (
      <div className="max-w-3xl mx-auto p-10 text-center">
        ...
        <p className="text-gray-600 mb-6">The content you're looking for doesn't exist or isn't published.</p>
        <Link to="/articles" className="px-4 py-2 bg-blue-600 text-white rounded">Return to Home</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      {item.content_type === 'article' && item.rich_text_content && (
        <div className="prose" dangerouslySetInnerHTML={{ __html: item.rich_text_content }} />
      )}

      {item.content_type === 'podcast' && audioSrc && (
        <div className="mt-6">
